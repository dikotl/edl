...
public class IfMacro : IMacro
{
    ...
    private static ICommand[] InlineIfLambda(Ast.Expression expression)
    {
        if (expression is Ast.Function function)
        {
    ...
    private static void ValidateArgs(
        Ast.Expression[] args,
        out Ast.Expression condition,
        out Ast.Expression thenExpression,
        out Ast.Expression? elseExpression)
    {
        ...
        if (condition is Ast.Function)
        {
            throw new Exception($"Intrinsic 'if' expects the first argument not to be a function.");
        }

        if (thenExpression is Ast.Function thenBlock && thenBlock.Parameters.Length > 0)
        {
        ...
        if (args.Length == 4)
        {
            ...
            if (args[2] is not Ast.Identifier identifier || identifier.Name != "else")
            {
                throw new Exception($"Intrinsic 'if' expects third argument to be identifier 'else', got {args[2]}.");
            }
            if (elseExpression is Ast.Function elseBlock && elseBlock.Parameters.Length > 0)
            {
